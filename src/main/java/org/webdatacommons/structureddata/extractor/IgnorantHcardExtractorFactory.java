...
class IgnorantHcardExtractor extends EntityBasedMicroformatExtractor {
	...
	public ExtractorDescription getDescription() {
		return IgnorantHCardExtractorFactory.getDescriptionInstance();
	}
	...
	private void fixIncludes(HTMLDocument document, Node node) {
		...
		if ("TD".equals(node.getNodeName())
				&& (null != attributes.getNamedItem("headers"))) {
		...
		for (Node current : document.findAll("//*[@class]")) {
			...
			if (null == id)
				continue;
			id = new TextField(StringUtils.substringAfter(id.value(), "#"),
					id.source());
	...
	protected boolean extractEntity(Node node, ExtractionResult out)
		...
		foundSomething |= addStringProperty("sort-string", card,
				vCARD.sort_string);
		...
		tser.addResourceRoot(DomUtils.getXPathListForNode(node), card,
				this.getClass());
	...
	private boolean addTelephones(Resource card) {
		...
		for (Node node : fragment.findAll(".//*[contains(@class,'tel')]")) {
			...
			if (values.length == 0) {
				// no sub values
				String[] typeAndValue = telFragment.getSingularUrlField("tel")
						.value().split(":");
				...
				HTMLDocument.TextField[] types = telFragment
						.getPluralTextField("type");
				if (types.length == 0) {
					found |= addTel(card, "tel", StringUtils.join(valuesStr));
				}
				for (HTMLDocument.TextField type : types) {
					found |= addTel(card, type.value(),
	...
	private boolean addTel(Resource card, String type, String value) {
		...
		if (composed == null) {
			...
			if (simple == null) {
				return conditionallyAddResourceProperty(card, (IRI) vCARD.tel, (IRI) tel);
			}
	...
	private boolean addSubMicroformat(String className, Resource resource,
			IRI property) {
	...
	private boolean addStringProperty(String className, Resource resource,
			IRI property) {
		final HTMLDocument.TextField textField = fragment
				.getSingularTextField(className);
		return conditionallyAddStringProperty(textField.source(), resource,
				property, textField.value());
	}
	 ...
	 * @param resource
	 * @param property
	 * @return <code>true</code> if the multi property has been added,
	 *         <code>false</code> otherwise.
	 */
	private boolean addStringMultiProperty(String className, Resource resource,
			IRI property) {
		HTMLDocument.TextField[] fields = fragment
				.getPluralTextField(className);
		boolean found = false;
		for (HTMLDocument.TextField field : fields) {
			found |= conditionallyAddStringProperty(field.source(), resource,
	...
	private boolean addCategory(Resource card) {
		HTMLDocument.TextField[] categories = fragment
				.getPluralTextField("category");
		boolean found = false;
		for (HTMLDocument.TextField category : categories) {
			found |= conditionallyAddStringProperty(category.source(), card,
					vCARD.category, category.value());
		}
		return found;
	}
	...
	private boolean addUid(Resource card) {
		...
		return conditionallyAddStringProperty(fragment.getDocument(), card,
				vCARD.uid, uid.value());
	}
	...
	private boolean addLogo(Resource card) throws ExtractionException {
		...
		for (TextField link : links) {
			found |= conditionallyAddResourceProperty(card, vCARD.logo,
					getHTMLDocument().resolveIRI(link.value()));
		}
		return found;
	}
	...
	private void addFieldTriple(Node n, BNode bn, String fieldName,
			String fieldValue) {
		conditionallyAddLiteralProperty(n, bn, vCARD.getProperty(fieldName),
				valueFactory.createLiteral(fieldValue));
	}
	...
	private boolean addNames(Resource card) {
		...
		for (String fieldName : HCardName.FIELDS) {
			...
			if (name.isMultiField(fieldName)) {
				Collection<HTMLDocument.TextField> values = name
						.getFields(fieldName);
	...
	private boolean addFn(Resource card) {
		...
		return conditionallyAddStringProperty(fullNameTextField.source(), card,
				vCARD.fn, fullNameTextField.value());
	}
	...
	private boolean addOrganizationName(Resource card) {
		...
		conditionallyAddLiteralProperty(organizationTextField.source(), org,
		...
		if (organizationUnitTextField != null) {
			conditionallyAddStringProperty(organizationUnitTextField.source(),
					org, vCARD.organization_unit,
	...
	private boolean addUrl(Resource card) throws ExtractionException {
		...
		boolean found = false;
		for (TextField link : links) {
			found |= conditionallyAddResourceProperty(card, vCARD.url,
					getHTMLDocument().resolveIRI(link.value()));
		}
		return found;
	}
...
class IgnorantHCardExtractorFactory extends SimpleExtractorFactory<IgnorantHcardExtractor>{
    ...
    public IgnorantHCardExtractorFactory() {
        super(
                IgnorantHCardExtractorFactory.NAME,
                IgnorantHCardExtractorFactory.PREFIXES,
                Arrays.asList("text/html;q=0.1", "application/xhtml+xml;q=0.1"),
                "example-mf-hcard.html");
    }

    @Override
    public IgnorantHcardExtractor createExtractor() {
        return new IgnorantHcardExtractor();
    }
