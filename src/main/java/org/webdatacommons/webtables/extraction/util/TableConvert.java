...
public class TableConvert {
	...
	public Element[][] transpose (Element[][] array, Element table) {
		Elements tableRows = table.getElementsByTag("tr");
		int tableHeight = tableRows.size();
		...
		for (int tr_idx = 0; tr_idx < tableHeight; tr_idx++) {
		  ...
		  Element[][] array_new = new Element[tableWidth][tableHeight];
	...
	public static void main(String[] args) {
		String url = "F:\\MSc. Business Informatics - Uni Mannheim\\HiWi Job\\allGoldStandardFiles\\989567_8444333838044559462_verticaltablecheck";
		InputStream in;
		...
		try {
			...
			for (Element aTable : doc.getElementsByTag("table")) {
				...
				if(subtables.size() == 0) {
					...
					if (result.isPresent()){
						Element[][] newResult = tableConvert.transpose(result.get(), aTable);
						tableConvert.printTable(newResult);
					}
					else
