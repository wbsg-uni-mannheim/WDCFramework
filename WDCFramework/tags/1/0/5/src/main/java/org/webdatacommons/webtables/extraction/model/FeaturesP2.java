...
public class FeaturesP2 {
	...
	private FastVector attributeVector; // vector of all atrributes PLUS class
										// attribute
	private FastVector classAttrVector; // vector of strings of all possible
										// class values
	...
	public FeaturesP2() {
		...
		for (String s : FeaturesP2.getFeatureNames()) {
			Attribute newAttr = new Attribute(s); // create new Feature with
													// name from whitelist
	...
	public Instance computeFeatures(Element[][] convertedTable) {
		...
		TableStats tStats = new TableStats(convertedTable[0].length,
				convertedTable.length);
		...
		for (tStats.rowIndex = 0; tStats.rowIndex < tStats.getTableHeight(); tStats.rowIndex++) {
			for (tStats.colIndex = 0; tStats.colIndex < tStats.getTableWidth(); tStats.colIndex++) {
				...
				// onCell event
				for (AbstractTableListener listener : globalListeners) {
					listener.computeCell(
							convertedTable[tStats.rowIndex][tStats.colIndex],
							tStats);
				}
		...
		for (int i = 0; i < localRowIndexes.length; i++) {
			...
			for (tStats.colIndex = 0; tStats.colIndex < tStats.getTableWidth(); tStats.colIndex++) {
				...
				for (AbstractTableListener listener : localListeners) {
					listener.computeCell(
							convertedTable[currentRowIndex][tStats.colIndex],
							tStats);
				}
			}
			...
			for (AbstractTableListener listener : localListeners) {
				...
				for (Map.Entry<String, Double> entry : results.entrySet()) {
					// insert as ORIGINAL_ATTRIBUTE_NAME_ROW_X where ROW_X is
					// the
					// specific row of the current loop
					resultMap.put(entry.getKey() + "_ROW_" + i,
							entry.getValue());
				}
		...
		for (int i = 0; i < localColIndexes.length; i++) {
			...
			for (tStats.rowIndex = 0; tStats.rowIndex < tStats.getTableHeight(); tStats.rowIndex++) {
				...
				for (AbstractTableListener listener : localListeners) {
					listener.computeCell(
							convertedTable[tStats.rowIndex][currentColIndex],
							tStats);
				}
			}
			...
			for (AbstractTableListener listener : localListeners) {
				HashMap<String, Double> results = listener.getResults();
				for (Map.Entry<String, Double> entry : results.entrySet()) {
					// insert as ORIGINAL_ATTRIBUTE_NAME_COL_X where COL_X is
					// the
					// specific column of the current loop
					resultMap.put(entry.getKey() + "_COL_" + i,
							entry.getValue());
				}
		...
		return Tools.createInstanceFromData(resultMap, attributeList,
				attributeVector);
	}
	...
	public class ContentRatios extends AbstractTableListener {
		...
		private double image_ratio, alphabetical_ratio, digit_ratio;
		...
		public void finalize() {
			image_ratio = (cellCount > 0) ? ((double) images / (double) cellCount)
					: 0.0;
			alphabetical_ratio = (cellCount > 0) ? ((double) alphabetical / (double) cellCount)
					: 0.0;
			digit_ratio = (cellCount > 0) ? ((double) digits / (double) cellCount)
					: 0.0;
		}
	...
	public class CumulativeContentTypeConsistency extends AbstractTableListener {
		private double ctc_sum_r, ctc_r; // Cumulative Length Consistency of
											// rows
		private double ctc_sum_c, ctc_c; // Cumulative Length Consistency of
											// columns
		...
		private ContentType getDominantType(List<ContentType> list) {
			...
			// put all occurrences of ContentTypes into a map together with
			...
			for (ContentType ct : frequencyMap.keySet()) {
				...
				if (currentCount > maxCount) {
					...
					// as the ContentType enum itself is sorted by priority of
					// the
					// content types compare the ordinal values

					// replace the dominant type with the current one only if
					// they both
					// have the same frequency but the current one has higher
					// priority
		...
		public void onCell(Element content, TableStats stats) {
			...
			// / ROWS - handle row end
			if (stats.colIndex == stats.getTableWidth() - 1) { // last column =
																// row end
																// reached
			...
			if (stats.rowIndex == stats.getTableHeight() - 1) { // last row =
																// column end
																// reached
	...
	public class LocalContentRatios extends AbstractTableListener {
		private int cellCount, count_th, count_anchor, count_img, count_input,
				count_select, count_contains_number, count_is_number,
				count_colon, count_comma;
		private double ratio_th, ratio_anchor, ratio_img, ratio_input,
				ratio_select, ratio_contains_number, ratio_is_number,
				ratio_colon, ratio_comma;
		...
		public void initialize(TableStats stats) {
			cellCount = count_th = count_anchor = count_img = count_input = count_select = count_contains_number = count_is_number = count_colon = count_comma = 0;
		}
		...
		public void onCell(Element content, TableStats stats) {
			...
			// thus they aren't determined via getContentType(...) - which is
			// exclusive
		...
		public void finalize() {
			ratio_th = (cellCount > 0) ? ((double) count_th / (double) cellCount)
					: 0.0;
			ratio_anchor = (cellCount > 0) ? ((double) count_anchor / (double) cellCount)
					: 0.0;
			ratio_img = (cellCount > 0) ? ((double) count_img / (double) cellCount)
					: 0.0;
			ratio_input = (cellCount > 0) ? ((double) count_input / (double) cellCount)
					: 0.0;
			ratio_select = (cellCount > 0) ? ((double) count_select / (double) cellCount)
					: 0.0;
			ratio_colon = (cellCount > 0) ? ((double) count_colon / (double) cellCount)
					: 0.0;
			ratio_contains_number = (cellCount > 0) ? ((double) count_contains_number / (double) cellCount)
					: 0.0;
			ratio_is_number = (cellCount > 0) ? ((double) count_is_number / (double) cellCount)
					: 0.0;
			ratio_comma = (cellCount > 0) ? ((double) count_comma / (double) cellCount)
					: 0.0;
		}
		...
		public HashMap<String, Double> getResults() {
			...
			result.put("LOCAL_RATIO_CONTAINS_NUMBER", new Double(
