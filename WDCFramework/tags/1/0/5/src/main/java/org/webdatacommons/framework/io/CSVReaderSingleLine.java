 ...
 * 
 */
public class CSVReaderSingleLine extends CSVReader implements Closeable {
	 ...
	 */
	public CSVReaderSingleLine(Reader reader) {
	 ...
	 */
	public CSVReaderSingleLine(Reader reader, char separator) {
	 ...
	 */
	public CSVReaderSingleLine(Reader reader, char separator, char quotechar) {
		...
		do {
			...
			for (int i = 0; i < nextLine.length(); i++) {
				...
				if (c == this.escape) {
					...
					sb = new StringBuilder(INITIAL_READ_SIZE); // start work on
																// next token
				} else {
					sb.append(c);
				}
